import argparse
import mimetypes
import os

from dotenv import load_dotenv
from google import genai
from google.genai import types

# setup gemini
load_dotenv()
api_key = os.environ.get("GEMINI_API_KEY")
if not api_key:
    raise RuntimeError("GEMINI_API_KEY environment variable not set")

# use api key to create a new instance of gemini client
client = genai.Client(api_key=api_key)

model = "gemma-3-27b-it"


def main():
    parser = argparse.ArgumentParser(description="Multimodal query rewriting")
    parser.add_argument("--image", type=str, required=True, help="Path to an image file")
    parser.add_argument("--query", type=str, required=True, help="Text query to rewrite based on the image")
    args = parser.parse_args()


    #use mimetypes.guess_type to determine the MIME type of the image
    mime, _ = mimetypes.guess_type(args.image)
    mime = mime or "image/jpeg"

    # open image file - read it in binary mode (rb)
    with open(args.image, "rb") as f:
        img = f.read()


    # gemini prompt
    prompt = """Given the included image and text query, rewrite the text query to improve search results from a movie database. Make sure to:
- Synthesize visual and textual information
- Focus on movie-specific details (actors, scenes, style, etc.)
- Return only the rewritten query, without any additional commentary"""

    # gemini request
    parts = [
        prompt,
        types.Part.from_bytes(data=img, mime_type=mime),
        args.query.strip(),
    ]
            
    # get response from gemini
    response = client.models.generate_content(model=model, contents=parts)

    print(f"Rewritten query: {response.text.strip()}") # type: ignore
    if response.usage_metadata is not None:
        print(f"Total tokens:    {response.usage_metadata.total_token_count}")

if __name__ == "__main__":
    main()